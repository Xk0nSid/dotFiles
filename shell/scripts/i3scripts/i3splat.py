...
def kitty(wdir="~", command=None, name="kitty", interactive=True):
    shell = os.environ["SHELL"]

    wdir = os.path.expanduser(wdir)
    args = ["kitty", "-d", shlex.quote(wdir), "--name", name, "-e", shell]
    if command != None:
        args.append("-c")
        args.append(shlex.quote("%s; %s -i" % (command, shell)))

    return App(command=_cmd(args), xClass="kitty", xInstance=name)
...
def vscode(paths):
    ...
    return App(command=_cmd(["code", "-n"] + quoted_paths), xClass="vscode")
